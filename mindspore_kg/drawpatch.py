...
def drawpatch(canvas, patch_size, angle, scale, location, grayscale):
    ...
    patch = GetParallel(distribution=distribution, height=patch_size[0], length=patch_size[1],
    ...
    m = np.minimum(imgRotation,
                   pad_canvas[Aligned_point[1]:Aligned_point[1] + h, Aligned_point[0]:Aligned_point[0] + w])
