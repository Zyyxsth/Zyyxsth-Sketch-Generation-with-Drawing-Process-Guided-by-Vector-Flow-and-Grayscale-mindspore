...
if __name__ == '__main__':
    ...
    if draw_new == True:
        ...
        for dirs in range(direction):
            ...
            gradient_magnitude = mindspore.ops.sqrt(x_der ** 2.0 + y_der ** 2.0)
                ...
                for i in np.squeeze(distensce).tolist():
                    if raw < h:
                        ...
                        for interval in get_start_end(mask[y - 2 * period] * dir_mask[y - 2 * period]):
                            ...
                            stroke_temp['importance'] = (255 - stroke_temp['grayscale']) * Tensor.sum(
                                gradient_norm[y:y + period, interval[0] + 2 * period:interval[1] + 2 * period]).numpy()
